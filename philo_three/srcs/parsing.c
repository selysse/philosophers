...
/*   Updated: 2021/02/10 23:44:32 by gselyse          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		create_sem(t_philo *philo)
{
	...
	if (!(name = ft_strdup("eat_philo")))
		return (EXIT_FAILURE);
	sem_unlink(name);
	if ((philo->eat_philo = sem_open(name, O_CREAT, 0644, 1)) == SEM_FAILED)
		return (EXIT_FAILURE);
	free(name);
	name = NULL;
	if (!(name = ft_strdup("count_sem")))
		return (EXIT_FAILURE);
	sem_unlink(name);
	if ((philo->count_sem = sem_open(name, O_CREAT, 0644, 0)) \
	== SEM_FAILED)
		return (EXIT_FAILURE);
	free(name);
	name = NULL;
